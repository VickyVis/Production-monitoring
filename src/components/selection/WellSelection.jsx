...
export default function Example() {
  const [selectedWell, setSelectedWell] = useState(Data[0])
  const [query, setQuery] = useState('')
 
  useEffect(() => {
   
    sendData(selectedWell);
  }, [selectedWell]);

  const filteredWell =
    query === ''
      ? Data
      : well.filter((wellItem) =>
          wellItem[selectedCategory]
            .toString()
            .toLowerCase()
            .replace(/\s+/g, '')
  ...
  return (
    ...
    <div className="fixed w-60 border-2 rounded-lg">
      ...
      <Combobox value={selectedWell} onChange={setSelectedWell}>
        <div className="relative mt-1">
          <Combobox.Input
            className="w-full border-none py-2 pl-3 pr-10 text-sm leading-5 text-gray-900 focus:ring-0"
            displayValue={(wellItem) => wellItem.name}
          ...
          >
            <Combobox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
              {filteredWell.length === 0 && query !== '' ? (
                <div className="relative cursor-default select-none px-4 py-2 text-gray-700">
                  Nothing found.
                </div>
              ) : (
                filteredWell.map((wellItem) => (
                  <Combobox.Option
                    key={wellItem.id}
                    className={({ active }) =>
